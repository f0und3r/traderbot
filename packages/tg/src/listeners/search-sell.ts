...
import { searchSellText } from "./welcome"
...
const listener: Listener = (bot, msg, state, updateState, next) => {
  ...
  if (state.type === "welcome" && text === searchSellText) {
    updateState({ type: "search-sell" })
    bot.sendMessage(chatId, inputIdText)
  } else if (state.type === "search-sell") {
