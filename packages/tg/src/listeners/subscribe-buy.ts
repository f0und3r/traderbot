...
const inputAmountText = "Введите минимальную сумму скупки (0 - любая)"
...
const listener: Listener = (bot, msg, state, updateState, next) => {
  ...
  if (state.type === "welcome" && text === subscribeBuyText) {
    updateState({ type: "subscribe-buy", id: null })
    bot.sendMessage(chatId, inputIdText)
  } else if (state.type === "subscribe-buy") {
    ...
    if (state.id === null && matches !== null) {
      ...
      if (itemId !== 0) {
        selectItems([itemId])
          .then(items => {
            if (items.length > 0) {
              updateState({ type: "subscribe-buy", id: itemId })
    ...
    if (state.id !== null && matches !== null) {
      const minAmount = parseInt(matches[0]) || 0

      updateState({ type: "welcome" })

      saveWatch(chatId, state.id, "buy", minAmount)
        .then(watchId => {
